"adaboost.M1" <-
function(formula, data,boos=TRUE, mfinal=100, coeflearn="Breiman",minsplit=5, cp=0.01,maxdepth=nlevels(vardep)) {
	...
	pond <- rep(0,mfinal) # Un vector donde guardaremos la ponderación de cada árbol.
	...
	#Creamos muestras boostrap utilizando los pesos
		...
		if (boos==TRUE) {
			...
			fit <- rpart(formula,data=data[boostrap,-1], minsplit=minsplit, cp=cp,maxdepth=maxdepth)
		...
		if (boos==FALSE) {
			...
			ind<-as.numeric(vardep != flearn) 	#Crear un vector indicador
			err<- sum(pesos*ind)         #Calcula el error ponderado en esa iteración
		...
		# Según Opitz y Maclin si no 0<err<0.5 se ajustan los valores de c.
